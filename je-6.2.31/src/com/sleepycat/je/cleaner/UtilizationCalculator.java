 ...
 *
 * LN size adjustments are performed because the obsolete size of LNs may be
 * unknown (if it was written prior to JE 6.0) when we delete or update an LN
 * that is not resident in cache.  We do not wish to fetch the LN just to
 * obtain the size of the previous version. Therefore we record in the
 * FileSummary that an LN became obsolete (by incrementing obsoleteLNCount) and
 * also that its size was not counted (by not incrementing
 * obsoleteLNSizeCounted and not adding a size to obsoleteLNSize).
 * 
...
public class UtilizationCalculator {
     ...
     */
    synchronized boolean adjustUtilization(
        long fileNum,
        long endFileNum,
        ...
        final int estObsLNCount = Math.min(
            estimatedFileSummary.obsoleteLNCount,
            estimatedFileSummary.totalLNCount);
        final int estObsLNSize = Math.min(
            estimatedFileSummary.obsoleteLNSize,
            estimatedFileSummary.totalLNSize);
        final int estObsLNSizeCounted = Math.min(
            estimatedFileSummary.obsoleteLNSizeCounted, estObsLNCount);
        final int trueObsLNSize = Math.min(
            trueFileSummary.obsoleteLNSize, trueFileSummary.totalLNSize);
