...
public class SortedLSNTreeWalker {
            ...
            if (in.isEntryPendingDeleted(i) ||
                ...
                try {
                    if (node.isBINDelta(false)) {
                        ...
                        addToLsnINMap(fullLsn, in, i, delta, lsn);
                        ...
                        processResidentChild(
                            lsn, node, lnKey, in.getLastLoggedSize(i), pendingLSNs);
                    }
                } finally {
                    if (isLatched) {
                        node.releaseLatch();
     ...
     * sorted LSN order.
     */
    private Pair<Node, Integer> fetchLSN(long lsn,
                                         DatabaseEntry lnKeyEntry,
                                         LSNAccumulator pendingLSNs)
        throws FileNotFoundException, DatabaseException {
        ...
        try {
            ...
            WholeEntry wholeEntry =
                envImpl.getLogManager().getWholeLogEntry(lsn);
            ...
            if (entry instanceof BINDeltaLogEntry) {
                ...
                long fullLsn = delta.getPrevFullLsn();
                pendingLSNs.add(fullLsn);
                addToLsnINMap(fullLsn, in, index, delta.getMainItem(), lsn);
                return null;
            }
            ...
            if (deltaObject != null) {
                ...
                if (deltaObject instanceof OldBINDelta) {
                    ...
                    assert lsn == delta.getLastFullVersion();
                    delta.reconstituteBIN(dbImpl, fullBIN);
                    lastLoggedLsn = inEntry.getDeltaLsn();
                }
            }
