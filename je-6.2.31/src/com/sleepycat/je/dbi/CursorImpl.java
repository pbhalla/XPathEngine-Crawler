...
public class CursorImpl implements Cloneable {
    ...
    public boolean hasDuplicates() {
        return databaseImpl.getSortedDuplicates();
    }
    ...
    /**
     * For a non-sticky cursor, this is an alternative to closing or resetting
     * the cursor when it is initialized and an advancing operation
     * (next/prev/skip) is about to be performed. The cursor position cannot be
     * reset as it would be if the operation were a search, for example.
     */
    public void beforeAdvance() {
         ...
         *
         * Note that EVICT_BIN cannot be implemented for a non-sticky cursor.
         * We would have to call removeCursor before the BIN eviction, and then
         * our position would be unstable. Even if this were possible, since
         * we can't predict the new position we would evict the BIN at every
         * operation, even when the BIN remained the same.
         *
         * It might be possible to support MAKE_COLD here, but more thought and
         * testing would be required.
         *
         * Therefore, when EVICT_BIN or MAKE_COLD are configured, cursor
         * cloning is always used.
         */
        if (cacheMode == CacheMode.EVICT_LN && bin != null) {
            evict();
        }
    ...
    private void removeCursorAndPerformCacheEviction(CursorImpl newCursor) {
        ...
        try {
            ...
            bin.removeCursor(this);
            performCacheEviction(newCursor); // may release latch
        } finally {
            releaseBIN();
            clear();
        }
    ...
    public void criticalEviction() {
        ...
        if (allowEviction &&
            ...
            databaseImpl.getEnv().criticalEviction(false /*backgroundIO*/);
        }
    }
    ...
    private void performCacheEviction(final CursorImpl newCursor) {
        ...
        case MAKE_COLD:
            ...
            } else if (!envImpl.isCacheFull()) {
                /* 
                 * The BIN has changed, but the cache is not full. Just evict
                 * the LN.
                 */
                evict(true);
            } else {
    ...
    /**
     * Evict current BIN.  Must already be latched. The latch will be released
     * inside the doEvictOneIN() call.
     */
    private void evictBIN() {
        databaseImpl.getEnv().getEvictor().doEvictOneIN(
    ...
    public void evict()
        ...
        evict(false); // isLatched
    }

    /**
     * Evict the LN node at the cursor position.
     */
    public void evict(boolean isLatched)
        throws DatabaseException {
    ...
    public OperationStatus deleteCurrentRecord(
        ...
        try {
             ...
             */
            final int lastLoggedSize = bin.getLastLoggedSize(index);
            LN ln;
            if ((lastLoggedSize == 0 &&
                 envImpl.getCleaner().getFetchObsoleteSize(databaseImpl)) ||
            ...
            /*
             * Make the existing LN deleted, if cached, else create a new
             * deleted LN.
             */
            final long oldLNMemSize;
            if (ln != null) {
                /* Leave LN cached. Set ln.data to null and mark it dirty. */
                oldLNMemSize = ln.getMemorySizeIncludedByParent();
                ln.delete();
            } else {
                /*
                 * LN is not cached. Create an LN with ln.data == null, and
                 * mark it dirty. Then attach it to the tree.
                 */
                ln = dbType.createDeletedLN(envImpl);
                oldLNMemSize = ln.getMemorySizeIncludedByParent();
                bin.attachNode(index, ln, null /*lnSlotKey*/);
            }
            ...
            logResult = ln.optionalLog(
                envImpl, databaseImpl, bin.getKey(index), oldLsn,
                lastLoggedSize, locker, wli, repContext);
             ...
             */
            bin.updateEntry(
                index, oldLNMemSize, logResult.newLsn, logResult.newSize,
                null /*lnSlotKey*/);
            bin.setPendingDeleted(index);

            /* Cache record version for delete operation. */
            setCurrentVersion(ln.getVLSNSequence(), logResult.newLsn);
        ...
        trace(Level.FINER, TRACE_DELETE, bin, index, oldLsn, logResult.newLsn);
    ...
    private Pair<LockStanding, Boolean> insertRecordInternal(
         ...
         * inserts databaseImpl in the undoDatabases map. Noop for other
         * non-HA lockers.
         */
        ...
        int insertIndex = bin.insertEntry1(
            ln, key, DbLsn.NULL_LSN, blindInsertion);
        ...
        try {
            logResult = ln.optionalLog(
                envImpl, databaseImpl, key,
                DbLsn.NULL_LSN /*oldLSN*/, 0 /*oldSize*/,
                locker, wli, repContext);
        } finally {
        ...
        if (lockStanding == null) {
            ...
            /* For DW DBs, set the slot LSN to NULL. Otherwise, noop. */
             ...
             */
            bin.updateEntry(index, ln, logResult.newLsn, logResult.newSize, key);
        ...
        traceInsert(Level.FINER, bin, logResult.newLsn, index);
    ...
    private OperationStatus updateRecordInternal(
        ...
        final long oldLNMemSize;
        if (ln != null) {
            /* LN is cached: set ln.data to newData and mark ln dirty. */
            oldLNMemSize = ln.getMemorySizeIncludedByParent();
            ln.modify(newData);
        } else {
            /* LN is not cached, create new LN for logging. */
            ln = dbType.createUpdatedLN(envImpl, newData);
            /* Make new LN resident. */
            bin.attachNode(index, ln, null /*lnSlotKey*/);
            oldLNMemSize = ln.getMemorySizeIncludedByParent();
        }
        ...
        logResult = ln.optionalLog(
            envImpl, databaseImpl, (key != null ? key : oldKey),
            oldLsn, lastLoggedSize, locker, wli, repContext);
         ...
         */
        bin.updateEntry(
            index, oldLNMemSize, logResult.newLsn, logResult.newSize, key);
         ...
         * should not evict a dirty LN since it may be logged unnecessarily.
         */
        if (databaseImpl.getSortedDuplicates() &&
            !databaseImpl.isDeferredWriteMode() &&
            bin.getTarget(index) != null) {
        ...
        trace(Level.FINER, TRACE_MOD, bin, index, oldLsn, logResult.newLsn);
    ...
    public boolean positionFirstOrLast(boolean first)
        ...
        try {
            if (first) {
                bin = databaseImpl.getTree().getFirstNode(cacheMode);
            } else {
                bin = databaseImpl.getTree().getLastNode(cacheMode);
            }
    ...
    public int searchRange(
        ...
        try {
            ...
            bin = databaseImpl.getTree().search(
    ...
    public LockStanding searchExact(
        ...
        try {
            ...
            bin = databaseImpl.getTree().search(key, cacheMode);
     ...
     *
     *  4a. If dataRequested is false, we wait until the deleting txn finishes.
     ...
     *
     *  4b. If dataRequested is true, then we check whether the deleting txn is
    ...
    private LockStanding lockLNAndCheckDeleted(
        ...
        assert !(dataRequested && databaseImpl.getSortedDuplicates());
    ...
    public void getCurrent(
         ...
         */
        final boolean isEmptyLN = databaseImpl.isLNImmediatelyObsolete();

        final boolean dataRequested = 
            (foundData != null &&
             (!foundData.getPartial() || foundData.getPartialLength() != 0));

        final LN ln = (!isEmptyLN && dataRequested ?
        ...
        if (dataRequested) {
            ...
            byte[] lnData =
                (ln != null ? ln.getData() : LogUtils.ZERO_LENGTH_BYTE_ARRAY);
    ...
    public RecordVersion getCurrentVersion(boolean allowFetch) {
        ...
        if (currentRecordVersion != null) {
            if ((currentRecordVersion.getVLSN() !=
                ...
                !allowFetch ||
                !databaseImpl.getEnv().getPreserveVLSN()) {
                return currentRecordVersion;
            }
        }
    ...
    public static class LockStanding {
         ...
         * the LN. It returns a WriteLockInfo that is meant to be passed to
         * the LN logging method, where its info will be included in the LN
         * log entry and also copied into the new WriteLockInfo that will be
         * created for the new LSN.
         *
     ...
     */
    public LockStanding lockLN(final LockType lockType,
         ...
         */
        if (allowUncontended &&
            databaseImpl.getEnv().getTxnManager().getLockManager().
    ...
    public void checkEnv() {
        databaseImpl.getEnv().checkIfInvalid();
    }
