...
public class EnvironmentConfig extends EnvironmentMutableConfig {
     ...
     * unknown record sizes.
     *
     * Prior to JE 6.0, this parameter was true by default, because the size of
     * the last logged version of each record was not stored in the Btree,
     * sometimes resulting in utilization inaccuracies described below. In JE
     * 6.0 and later, the last logged size is stored in the Btree BINs (bottom
     * internal nodes), so that utilization can be calculated correctly during
     * record updates and deletions, while still avoiding a fetch of the old
     * version of the record.
     * <p>
     * Therefore this parameter is false by default.  This config parameter is
     * still provided to allow enabling adjustments in particular cases where
     * it may benefit a particular application that has written data with
     * versions of JE prior to 6.0. However, the need for this parameter is not
     * expected to arise, and in our experience the utilization adjustments are
     * not beneficial to most applications.  For safety's sake adjustments can
     * still be enabled in this release, but they will be completely disabled
     * in a future version of JE.
     * <p>
     * Prior to JE 6.0, when a record is updated or deleted without first
     ...
     * </ul>
     * <p>
     * When setting this parameter to true, please be aware of the possibility
     * that undesired automatic adjustments may occur.  Because the adjustments
     * are based on sampled information and application workloads may change
     * over time, the adjustments are not always beneficial.  Utilization may
     * be adjusted incorrectly, causing undesired over-cleaning or
     * under-cleaning.  It is for this reason that the logged size of each
     * record is now stored in the Btree BINs (as of JE 6.0 or later) and this
     * parameter is false by default.
     * <p>
     * When this parameter is false (the default), the last two steps above are
     * not applied.  The LNSizeCorrectionFactor will still be calculated when
     * log files are cleaned and will be available via the {@link
     ...
     * be cleaned based on the estimated utilization alone.  In a future
     * version of JE, because the last logged record size is now stored in the
     * BINs and the estimated utilization is now accurate, the
     * LNSizeCorrectionFactor and related facilities will be removed entirely.
     * <p>
     * To guarantee that utilization will be calculated accurately for all
     * workloads and that automatic adjustments will not interfere, {@link
     * #CLEANER_FETCH_OBSOLETE_SIZE} should be set to true and {@link
     * #CLEANER_ADJUST_UTILIZATION} should be left set to false.
     *
     * <p><table border="1">
     * <tr><td>Name</td><td>Type</td><td>Mutable</td><td>Default</td></tr>
     * <tr>
