...
public class BIN extends IN {
    ...
    public void setKnownDeletedClearAll(int index) {
        ...
        setLsnInternal(index, DbLsn.NULL_LSN);
    }
     ...
     * So, this method will set this.lastDeltaVarsion. Otherwise, if the
    ...
    byte[] createBloomFilter() {
        ...
        for (int i = 0; i < getNEntries(); ++i) {
            ...
            byte[] suffix = entryKeyVals.get(i);
            if (suffix == null) {
    ...
    private void resetContent(
        ...
        entryKeyVals = new INKeyRep.Default(capacity);
        ...
        for (int i = 0; i < newNEntries; i += 1) {
            ...
            entryKeyVals = entryKeyVals.set(i, keys[i], this);
    ...
    private void resetContent(final BIN other) {
        ...
        entryKeyVals = other.entryKeyVals;
    ...
    void applyDelta(
        ...
        if (foundIndex >= 0 && (foundIndex & IN.EXACT_MATCH) != 0) {
             ...
             */
            updateEntry(foundIndex, child, lsn, lastLoggedSize, state);
            ...
            final int result = insertEntry1(
                child, key, lsn, state, false/*blindInsertion*/);
