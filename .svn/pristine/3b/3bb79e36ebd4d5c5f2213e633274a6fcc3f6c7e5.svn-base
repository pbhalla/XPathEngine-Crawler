    ...
    public void testKeySizeChange_IN_updateEntry() {
        commonKeySizeChange(new ChangeKey() {
            public void changeKey(final BIN bin,
                                  ...
                                  byte[] newKey) {
                bin.updateEntry(
                    index, bin.getTarget(index), bin.getLsn(index),
                    bin.getLastLoggedSize(index), newKey);
            }
        });
    }
    ...
    public void testKeySizeChange_IN_updateNode1() {
        ...
        commonKeySizeChange(new ChangeKey() {
            ...
            public void changeKey(final BIN bin,
                                  final int index,
                                  byte[] newKey) {
                final Node target = bin.fetchLN(index, CacheMode.UNCHANGED);
                assertEquals(Type.MAX_KEY_SIZE, bin.getKeyVals().getType());
                bin.updateEntry(
                    index,
                    target.getMemorySizeIncludedByParent(),
                    bin.getLsn(index),
    ...
    public void testKeySizeChange_IN_updateNode2() {
        commonKeySizeChange(new ChangeKey() {
            public void changeKey(final BIN bin,
                ...
                assertEquals(Type.MAX_KEY_SIZE, bin.getKeyVals().getType());
                bin.attachNode(index, target, newKey);
            }
        });
    }
