...
public final class Tree implements Loggable {
    ...
    public void delete(
        ...
        if (branchRoot != null) {
            ...
            if (!database.isDeferredWriteMode()) {
                 ...
                 */
                branchRoot.accountForSubtreeRemoval(inList, localTracker);
            }
    ...
    private void cascadeUpdates(ArrayList<SplitInfo> nodeLadder)
        ...
        for (int i = nodeLadder.size() - 1; i >= 0; i -= 1) {
            ...
            if (info.parent.isDbRoot()) {
                assert i == 0;
                final long newLsn = info.parent.optionalLog(logManager);
                ...
                final long newLsn = info.parent.optionalLogProvisional(
                    logManager, nextInfo.parent);

                nextInfo.parent.updateEntry(
                    nextInfo.index, newLsn, 0 /*lastLoggedSize*/);
            }
        }
    }
    ...
    public IN getNextIN(IN prevIn, boolean forward, CacheMode cacheMode) {
        ...
        assert prevIn.isLatchExclusiveOwner() : "what?";
                ...
                if (moreEntriesThisIn) {
                    ...
                    if (nextIN.getLevel() == targetLevel) {
                        ...
                        final IN ret = searchSubTree(
                            ...
                            targetLevel, cacheMode, null /*comparator*/);
     ...
     * been retieved from the LRU), its parent must be found and EX-latched;
     * however, if the C has been evicted already by another thread, there is
     * nothing to to (C will GC-ed).
     * or
     ...
     * implements two additional use cases via the extra "requireExactMatch"
     * param.
     *
     * (a) requireExactMatch == false && doFetch == false
     * In this case we are actually looking for the lowest cached ancestor
     * of the C node. The method will always return a node (considred as the
     ...
     *
     * (b) requireExactMatch == false && doFetch == true
     * This search mode is used during recovery. It seems that
     * requireExactMatch can actually be true here ???? (and maybe use a
     * targetLevel as well ????).
     *
     * The exclusiveLevel param:
     * In general, if exclusiveLevel == L, nodes above L will be SH latched and
     * nodes at or below L will be EX-latched. In all current use cases, except
     * case (a) above, L is set to 1 + C.level; in case (a) it is set to 0.
     * This is because in all use cases except (a), the parent P, if any,
     * should be at level 1 + C.level and since P should be returned
     * EX-latched, when the search reaches L, it must EX-latch the node at that
     * level. In case (a), the "parent" does not need to be EX-latched (and it
     * can in fact be at any level).
     */
    public SearchResult getParentINForChildIN(
        long targetNodeId,
        byte[] targetKey,
        ...
        if (result.parent != null) {
            ...
            assert((!doFetch && !requireExactMatch) ||
                   result.parent.isLatchExclusiveOwner());
        }
    ...
    private IN forceSplit(byte[] key, CacheMode cacheMode)
        ...
        try {
            ...
            LogManager logManager = database.getEnv().getLogManager();
                ...
                if (child.needsSplitting()) {
                    ...
                    if (parent.isDbRoot()) {
                        root.updateLsnAfterOptionalLog(
                            database, parent.getLastLoggedVersion());
                    }
                } else {
                    if (startedSplits) {
                        ...
                        if (lastParentForSplit == child.getNodeId()) {
                            newChildLsn = child.getLastLoggedVersion();
                        } else {
                            newChildLsn = child.optionalLogProvisional(
                                logManager, parent);
                        }

                        parent.updateEntry(
                            index, newChildLsn, 0 /*lastLoggedSize*/);
    ...
    private void splitRoot(CacheMode cacheMode)
        ...
        try {
            ...
            try {
                curRootLsn = curRoot.optionalLogProvisional(
                    logManager, newRoot);
    ...
    private IN searchSubTree(
        ...
        try {
            do {
                ...
                /* Latch the child. Note: BINs are always latched exclusive. */
                if (doGrandparentLatching && child.getLevel() != targetLevel) {
                    child.latchShared(cacheMode);
                } else {
                    child.latch(cacheMode);
                }
