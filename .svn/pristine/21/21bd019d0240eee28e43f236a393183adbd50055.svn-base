...
public class Cleaner implements DaemonRunner, EnvConfigObserver {
    ...
    private void processPendingLN(long originalLsn,
        ...
        try {
            ...
            LockResult lockRet = locker.nonBlockingLock(
                originalLsn, LockType.READ, false /*jumpAheadOfWaiters*/, db);
            ...
            processedHere = false;
            migratePendingLN(
                db, bin.getLsn(index), bin, index, originalLsn,
                true /*backgroundIO*/, CLEAN_PENDING_LN);
            completed = true;
     ...
     */
    private void migratePendingLN(DatabaseImpl db,
                                  long lsn,
                                  BIN bin,
                                  int index,
        ...
        try {
            ...
            if (!bin.isEntryKnownDeleted(index)) {
                ...
                if (ln == null) {
                    /* If fetchLN returns null, a deleted LN was cleaned.*/
            ...
            final LN.LogResult logResult = ln.log(
                env, db, bin.getKey(index), lsn, bin.getLastLoggedSize(index),
                null /*locker*/, null /*writeLockInfo*/, backgroundIO,
            ...
            CursorImpl.lockAfterLsnChange
                (db, lsn, logResult.newLsn, locker /*excludeLocker*/);
            migrated = true;
            completed = true;
            return;
        } finally {
