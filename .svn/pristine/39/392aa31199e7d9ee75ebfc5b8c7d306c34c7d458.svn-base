 ...
 *
 * The use of BIN-deltas has a big impact on slot compression because slots
 * cannot be compressed until we know that a full BIN will next be logged.  If
 * a slot were compressed prior to logging a BIN-delta, the record of the
 ...
 * when a BIN-delta will next be logged, and queued compression is used only
 * when a full BIN will next be logged.  Because, in general, BIN-deltas are
 ...
 *
 * + Because BIN-deltas are not used for DeferredWrite mode, queued compression
 ...
 *    Locker.addDeleteInfo, which determines whether a BIN-delta will next be
 ...
 *    avoid splitting when compression is deferred due to BIN-deltas.
 *
...
public class INCompressor extends DaemonThread {
    ...
    public BIN searchForBIN(DatabaseImpl db, BINReference binRef) {
        return (BIN) db.getTree().search(binRef.getKey(), CacheMode.UNCHANGED);
    }
