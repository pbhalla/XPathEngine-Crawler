...
public class LNFileReaderTest extends TestBase {
    ...
    private long createLogFile(int numIters, boolean trackLNs, boolean redo)
        ...
        for (int i = 0; i < numIters; i++) {
             ...
             */
            lsn = ln.log(
                envImpl, dbImpl, key, DbLsn.NULL_LSN, 0, userTxn,
                new WriteLockInfo(), false,
                ReplicationContext.NO_REPLICATE).newLsn;
            ...
            nonTxnalLN.log(
                envImpl, dbImpl, key, DbLsn.NULL_LSN, 0, null, null, false,
                ReplicationContext.NO_REPLICATE);

            /* Add a MapLN. */
            MapLN mapLN = new MapLN(dbImpl);
            lsn = mapLN.log(
                envImpl, mapDbImpl, key, DbLsn.NULL_LSN, 0, userTxn,
                new WriteLockInfo(), false,
                ReplicationContext.NO_REPLICATE).newLsn;
            if (!trackLNs) {
