 ...
 *
 * Note that as of JE 6.0, utilization adjustments should no longer be needed
 * because the last logged size is stored in the BIN.  Adjustments are
 * therefore disabled by default.  Adjustments are supported (can be enabled
 * explicitly) in the current release, to be safe, but will be removed entirely
 * in a future release as described in the JE 6 change log and the javadoc for
 * {@link com.sleepycat.je.EnvironmentConfig#CLEANER_ADJUST_UTILIZATION}.
 * See [#22275] for more information.
 *
 * Therefore, as of JE 6.0 this test is somewhat artificial because it uses a
 * test-only mode (BIN.TEST_NO_LAST_LOGGED_SIZES) to prevent storage of the
 * last logged size.  However, there is still some value here:
 *
 * 1) The tests that set forceNoLastLoggedSizes to true check that adjustments
 * and probes work when the size is not available because the data was written
 * with an older release, and the user has enabled adjustments explicitly.
 *
 * 2) The other tests (where forceNoLastLoggedSizes is false) ensure that
 * the obsolete sizes are known and correct, without any adjustments, even
 * though the CacheMode.EVICT_LN is used.  This helps to test that the last
 * logged size is correctly stored in the BIN.
 */
...
public class UtilizationCorrectionTest extends CleanerTestBase {
    ...
    private Database db;
    private boolean forceNoLastLoggedSizes;
    private boolean adjustUtilization = true;
    ...
    public static List<Object[]> genParams() {
        return getEnv(new boolean[] {false});//{false, true});
    }
    ...
    public void tearDown() 
        ...
        super.tearDown();
        BIN.TEST_NO_LAST_LOGGED_SIZES = false;
        db = null;
        env = null;
        envImpl = null;
        calculator = null;
    ...
    private void openEnv(EnvironmentConfig envConfig) {
        BIN.TEST_NO_LAST_LOGGED_SIZES = forceNoLastLoggedSizes;
        envConfig.setAllowCreate(true);
        /* LNs in cache are no longer used to store the last log size. */
        ...
        envConfig.setConfigParam(EnvironmentConfig.CLEANER_ADJUST_UTILIZATION,
                                 String.valueOf(adjustUtilization));
    ...
    /**
     * When the last logged size is stored in the BIN, obsolete sizes are
     * recorded correctly. Estimated and true utilization are the same, around
     * 0%, so the correction factor is around 1.00.
     *
     ...
     * When the last logged size is stored in the BIN, obsolete sizes are
     * recorded correctly. Estimated and true utilization are the same, around
     * 99%, so the correction factor is around 1.00.
     *
     ...
     * When the last logged size is stored in the BIN, obsolete sizes are
     * recorded correctly. Estimated and true utilization are the same, around
     * 1%, so the correction factor is around 1.00.
     *
     * With minUtilization=40, cleaning will occur naturally and will correct
     * utilization.  An adjustment occurs for all log files because they are
     * all cleaned.
     ...
     */
    @Test
    public void testDisableAdjustmentDeleteSmall() {
        minUtilization = 40;
        forceNoLastLoggedSizes = true;
        adjustUtilization = false;
        openEnv();
        envImpl.forceLogFileFlip();
        expectCorrection(1.97F, 1.99F);
        writeMixedDeleteSmall();
    ...
    /**
     * Perform simple cleaning (copy of testDeleteAll) but fiddle with
